...
export function validateSkillConventions(parsed, repoRoot) {
    const checks = [];
    ...
    const hasVerify = parsed.rawProse.includes("→ verify:") ||
    ...
    const isCritical = CRITICAL_PATH_PHASES.has(String(fm.effort ?? "") === "heavy" ? "Build" : "Build");
    const cap = isCritical ? 150 : 120;
    checks.push({
        id: "size-cap",
        pass: lineCount <= cap,
        message: lineCount <= cap
