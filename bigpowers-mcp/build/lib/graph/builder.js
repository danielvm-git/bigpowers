...
export function buildGraphFromSkills(parsedSkills) {
    const graph = { entities: new Map(), relations: [] };
    for (const skill of parsedSkills) {
        const fm = skill.frontmatter;
        const observations = [];
        if (fm.model)
            observations.push(`model: ${String(fm.model)}`);
        if (fm.effort)
            observations.push(`effort: ${String(fm.effort)}`);
        if (fm.description)
            observations.push(`description: ${String(fm.description)}`);
        const entity = {
            type: "entity",
            name: skill.name,
            entityType: "Skill",
...
export function mergeGraphs(base, incoming) {
