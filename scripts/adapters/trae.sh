#!/usr/bin/env bash
# story: e70s01
# scenario: SC-e70s01-P1-01
# Trae skill adapter — renders .trae/skills from SkillIR.

trae_is_unsafe_skill_name() {
  local name="$1"
...
trae_sanitize_name() {
  local name="$1"
  if trae_is_unsafe_skill_name "$name"; then
    echo "trae: invalid skill name: ${name:-<empty>}" >&2
    return 1
  fi
  printf '%s' "$name"
}
...
render_skill() {
  local safe_name
  safe_name=$(trae_sanitize_name "${IR_NAME:-}") || return 1
  local out="${TRAE_SKILLS:-.trae/skills}"
