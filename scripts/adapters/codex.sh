#!/usr/bin/env bash
# story: e65s01
# scenario: SC-e65s01-P1-01
# Codex CLI skill adapter — renders .codex/skills from SkillIR.

codex_is_unsafe_skill_name() {
  local name="$1"
...
codex_sanitize_name() {
  local name="$1"
  if codex_is_unsafe_skill_name "$name"; then
    echo "codex: invalid skill name: ${name:-<empty>}" >&2
    return 1
  fi
  printf '%s' "$name"
}
...
render_skill() {
  local safe_name
  safe_name=$(codex_sanitize_name "${IR_NAME:-}") || return 1
  local out="${CODEX_SKILLS:-.codex/skills}"
...
wire_context() {
  ...
  wire_context_mode config-bridge ".codex/config.toml" "instructions" "AGENTS.md"
}
