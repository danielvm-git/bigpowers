#!/usr/bin/env bash
# story: e72s01
# scenario: SC-e72s01-P1-01
# CodeBuddy skill adapter — renders .codebuddy/skills from SkillIR.

codebuddy_is_unsafe_skill_name() {
  local name="$1"
  [[ -z "$name" || "$name" == "null" || "$name" == *"/"* || "$name" == *".."* ]]
}

codebuddy_sanitize_name() {
  local name="$1"
  if codebuddy_is_unsafe_skill_name "$name"; then
    echo "codebuddy: invalid skill name: ${name:-<empty>}" >&2
    return 1
  fi
  printf '%s' "$name"
}
...
render_skill() {
  local safe_name
  safe_name=$(codebuddy_sanitize_name "${IR_NAME:-}") || return 1
  local out="${CODEBUDDY_SKILLS:-.codebuddy/skills}"
