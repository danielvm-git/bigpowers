#!/usr/bin/env bash
# story: e71s01
# scenario: SC-e71s01-P1-01
# Copilot CLI skill adapter — renders .copilot/skills from SkillIR.

copilot_is_unsafe_skill_name() {
  local name="$1"
  [[ -z "$name" || "$name" == "null" || "$name" == *"/"* || "$name" == *".."* ]]
}

copilot_sanitize_name() {
  local name="$1"
  if copilot_is_unsafe_skill_name "$name"; then
    echo "copilot: invalid skill name: ${name:-<empty>}" >&2
    return 1
  fi
  printf '%s' "$name"
}
...
render_skill() {
  local safe_name
  safe_name=$(copilot_sanitize_name "${IR_NAME:-}") || return 1
  local out="${COPILOT_SKILLS:-.copilot/skills}"
