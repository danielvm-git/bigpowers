#!/usr/bin/env bash
# story: e62s01
# scenario: SC-e62s01-P1-01
# OpenCode skill adapter — renders .opencode/skills from SkillIR.

opencode_is_unsafe_skill_name() {
  local name="$1"
  [[ -z "$name" || "$name" == "null" || "$name" == *"/"* || "$name" == *".."* ]]
}

opencode_sanitize_name() {
  local name="$1"
  if opencode_is_unsafe_skill_name "$name"; then
    echo "opencode: invalid skill name: ${name:-<empty>}" >&2
    return 1
  fi
  printf '%s' "$name"
}
...
render_skill() {
  local safe_name
  safe_name=$(opencode_sanitize_name "${IR_NAME:-}") || return 1
  local out="${OPENCODE_SKILLS:-.opencode/skills}"
