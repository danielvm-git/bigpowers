...
#
# Usage:
#   bash scripts/run-verification-gates.sh                    # deterministic gates only
#   bash scripts/run-verification-gates.sh --agent            # deterministic + agent stories
#   bash scripts/run-verification-gates.sh --agent --dry-run  # validate YAMLs only
#   bash scripts/run-verification-gates.sh --dry-run          # print plan only
#   bash scripts/run-verification-gates.sh --baseline         # run + pin baseline
...
for arg in "$@"; do
  case "$arg" in
    ...
    *) echo "Unknown flag: $arg"; echo "Usage: run-verification-gates.sh [--agent] [--dry-run] [--baseline] [--check-size]"; exit 2 ;;
  esac
done
