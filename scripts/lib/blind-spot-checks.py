#!/usr/bin/env python3
# story: e38s04
# blind-spot-checks.py — TEA-inspired heuristic blind-spot detector (Python engine).
# Called by scripts/check-blind-spots.sh. Not meant to be run standalone without args.
...
# Enumerate all tracked code files
...
for sid, sstatus in dev_status.items():
    if sstatus == "done":
        ...
        if not verify_file.exists():
            findings.append({
                "check": "verify-gap", "story_id": sid, "severity": "HIGH",
                "description": f"Story {sid} is marked done but no verification evidence at specs/verifications/{sid}-verify.yaml",
...
for sid, files in tagged_files.items():
    if dev_status.get(sid) in ("done", "active"):
        if not has_test_files(files):
            ...
            if code_only:
                findings.append({
                    "check": "test-gap", "story_id": sid, "severity": "MEDIUM",
                    "description": f"Story {sid} has {len(code_only)} tagged code file(s) with no matching test files",
                    "remediation": f"Add test coverage for: {', '.join(code_only[:3])}"
                })
    ...
    for epic_dir in sorted(EPICS_DIR.iterdir()):
        ...
        for task_file in sorted(epic_dir.glob("e*[0-9]-tasks.yaml")):
            ...
            if sstatus in ("done", "active") and sid not in tagged_files:
                findings.append({
                    "check": "epic-orphan", "story_id": sid, "severity": "LOW",
...
for sid in stale_from_matrix:
    findings.append({
        "check": "stale-tag", "story_id": sid, "severity": "LOW",
        "description": f"Story {sid} is marked done but still has story tags in code",
        "remediation": f"Remove `// story: {sid}` tags or confirm the story should remain done"
    })
...
for fpath, sids in file_story_counts.items():
    if len(sids) > 1:
        findings.append({
            "check": "double-tag", "file": fpath, "severity": "MEDIUM",
...
active_and_recent = [sid for sid in sorted(dev_status.keys()) if dev_status[sid] == "active"]
for sid in active_and_recent:
    if sid in tagged_files:
        ...
        if code_only and not has_test_files(code_files):
            findings.append({
                "check": "bootstrap-testless", "story_id": sid, "severity": "HIGH",
...
blind_spots = {
    ...
    "version": "1.0",
    "summary": {"total_findings": len(findings), "high": high_count, "medium": medium_count, "low": low_count},
    "findings": findings
}
