#!/usr/bin/env bash
# story: e38s09 e41s04 e45s02
# OKF kind-specific validators — sourced by validate-okf.sh

if [ -n "${VALIDATE_OKF_KINDS_LOADED:-}" ]; then return 0; fi
VALIDATE_OKF_KINDS_LOADED=1

val_nullish() { [ -z "$1" ] || [ "$1" = "null" ] || [ "$1" = "None" ]; }
val_nullish_or_empty() { val_nullish "$1" || [ "$1" = "[]" ]; }

okf_parse_frontmatter() {
  local file="$1"
  $PYTHON -c "
...
validate_story_metrics() {
  local file="$1" fm="$2" name="$3" errs=0 key val source generator commit_range eff
...
" 2>/dev/null)" || {
    local rc=$?
    if [ "$rc" -eq 1 ]; then printf "${RED}FAIL${NC} %s: effort.effort_hours missing or null\n" "$name"
    else printf "${RED}FAIL${NC} %s: effort_hours out of bounds\n" "$name"; fi
    EXIT_CODE=1; return
  }
...
validate_spec_migration() {
  local file="$1" fm="$2" name="$3" errs=0 key val fp_count
...
validate_migration_registry() {
  local file="$1" fm="$2" name="$3" count bad
...
validate_concept() {
  local file="$1" fm="$2" name="$3" errs=0 key val ref_count cat
...
validate_verification_report() {
  local file="$1" fm="$2" name="$3" errs=0 key val gs score rc
...
" 2>/dev/null)" || {
    rc=$?
    if [ "$rc" -eq 1 ]; then printf "${RED}FAIL${NC} %s: score missing or not numeric\n" "$name"
    else printf "${RED}FAIL${NC} %s: score out of bounds (0-100)\n" "$name"; fi
    EXIT_CODE=1; return
  }
...
validate_bundle() {
  local file="$1" name fm fm_type kind
  name="$(basename "$file")"
  fm="$(okf_parse_frontmatter "$file")" || {
