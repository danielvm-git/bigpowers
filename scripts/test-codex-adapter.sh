#!/usr/bin/env bash
# story: e65s01
# scenario: SC-e65s01-P1-02
# Regression tests for Codex CLI adapter render + hook templates.
set -euo pipefail
...
echo "=== test-codex-adapter.sh ==="

TMP=$(mktemp -d)
trap 'rm -rf "$TMP"' EXIT

if echo '{"name":"test-skill","description":"test desc","body":"test body"}' \
  | CODEX_SKILLS="$TMP" bash "$REPO_ROOT/scripts/adapters/codex.sh" \
...
if ! echo '{"name":"../escape","description":"d","body":"b"}' \
  | CODEX_SKILLS="$TMP/bad" bash "$REPO_ROOT/scripts/adapters/codex.sh" 2>/dev/null; then
  pass "rejects path traversal in skill name"
else
  fail "rejects path traversal in skill name"
fi

if [[ -f "$REPO_ROOT/scripts/hooks/codex/pre-tool-git-guard.sh" ]] \
  && [[ -f "$REPO_ROOT/scripts/hooks/codex/hooks-manifest.json" ]] \
  && [[ -f "$REPO_ROOT/scripts/hooks/codex/settings.example.json" ]]; then
...
BLOCK_OUT=$(echo '{"tool_input":{"command":"git push --force origin main"}}' \
  | bash "$REPO_ROOT/scripts/hooks/codex/pre-tool-git-guard.sh")
...
ADAPTER_OUT=$(bash "$REPO_ROOT/scripts/test-adapters.sh" codex 2>&1 || true)
if echo "$ADAPTER_OUT" | grep -q '0 failed'; then
  pass "test-adapters.sh codex"
else
  fail "test-adapters.sh codex"
  echo "$ADAPTER_OUT"
fi

echo "test-codex-adapter: $PASS passed, $FAIL failed"
[[ "$FAIL" -eq 0 ]] || exit 1
