...
(confirmed by grep at planning time). Once e33s01 ships skill-common.sh, these
...
Precondition: e33s01 complete — scripts/lib/skill-common.sh exists and its
...
6b. A script is sourced (not executed) by another script — sourcing order must
    avoid double-definition errors (library's include guard from e33s01).
...
- scripts/lib/skill-common.sh (e33s01, direction: in) — single source of path
  resolution.
- GitHub Actions workflows (perennial, direction: out) — invoke these scripts;
  behaviour must be unchanged.
- Downstream e39 (depends_on e33): e39's OKF generation scripts will follow
  this same source-the-library pattern on top of e33's render-target
...
### 18. Out of scope
- Refactoring sync-skills.sh's render logic (e33s03).
- Refactoring the index/lockfile scripts' skill-parsing internals (e33s04).
...
- specs/epics/e33-sync-pipeline/e33s01-skill-common-library.md (library this
  story consumes).
