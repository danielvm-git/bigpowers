...
The e37s01 spike proved an agent (DeepSeek v4 via the gh-aw Claude engine)
can execute a golden story headlessly — G-01 is live as
.github/workflows/e37-golden-deepseek.lock.yml. But the stories themselves
exist only as prose in QUALITY-GUARANTEE-STRATEGY.md §7. Without
machine-readable definitions, the --agent runner (e37s04) has nothing to
...
- Golden suite runner (system, e37s04) — parses the YAMLs and executes stories per-epic.
...
minimal-api fixture exists for g-01 (e37s02); e31 deterministic gates
shipped in v2.50.0.
...
6b. YAML unparseable — runner (e37s04) must fail loudly; authoring gate here
...
specs/benchmarks/reports/ (e37s04).
...
- specs/benchmarks/fixtures/minimal-api/ (e37s02, direction: in) — g-01 fixture target.
- scripts/run-golden-suite.sh --agent (e37s04, direction: out) — consumes these YAMLs.
- .github/workflows/e37-golden-deepseek.lock.yml (spike, direction: out) — G-01 workflow; the g-01.yaml definition becomes its source of truth.

### 10. Background processes
Not applicable — static definitions; execution scheduling is e37s04's concern.

### 11. Notifications
Not applicable — no runtime behaviour in this story.

### 12. Audit and logging
Not applicable — the YAMLs are version-controlled; run reports are e37s04 scope.
...
- Executing the stories or computing pass@k at runtime (e37s04).
- The G-02 off-by-one bug fixture and G-03/G-05 fixture repos — graders must
  ...
  minimal-api is deferred to e37s04 wiring.
...
- specs/epics/e37-golden-stories/epic.yaml (e37s03 verify command, flake policy note).
- .github/workflows/e37-golden-deepseek.lock.yml (spike output for G-01).
