...
if grep -q "HARD GATE\|HARD-GATE" skills/develop-tdd/SKILL.md \
   && grep -qi "before.*plan\|design.*approv\|plan.*approv\|no plan.*no code\|Do NOT write code" skills/develop-tdd/SKILL.md; then
  exit 0
fi
