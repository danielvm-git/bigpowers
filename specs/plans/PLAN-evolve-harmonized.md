...
Phase 0: HARD GATES Reference (pre-requisite for all skill edits)
  ├── Create specs/plans/HARD-GATES-REFERENCE.md
...
```
□ 1. Update output paths (specs/requirements/ → specs/product/, specs/plans/ → specs/tech-architecture/)
...
□ 7. Verify: grep "specs/requirements/\|specs/plans/" <skill>/SKILL.md returns 0 matches
```
...
□ 2. Verify: grep "specs/requirements/\|specs/plans/" <skill>/SKILL.md returns 0 matches
```
...
Each skill gets 1–3 HARD GATE callouts relevant to its phase. The reference document (`specs/plans/HARD-GATES-REFERENCE.md`) defines canonical patterns:
...
grep -c "specs/requirements/\|specs/plans/" skills/<name>/SKILL.md
# Expected: 0
...
Scenario: No legacy paths remain
  ...
  When grep runs for "specs/requirements/" or "specs/plans/" across all skills
  Then zero matches are returned

Scenario: Evolved structure is the default scaffold
  Given a new project initialized with seed-conventions
  Then specs/product/ exists (not specs/requirements/)
  And specs/tech-architecture/ exists (not specs/plans/)
